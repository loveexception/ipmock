...
/**
 * 枚举 ：t_subtopics status<br/>
 * 值：1:未执行,9:已发布,3:服务失败
 * @author hanbing
 *
 */
public enum SubtopicsStatus{
	...
	private SubtopicsStatus(String eVal,String eShow){
		this.enumVal = eVal;
		this.enumShow = eShow;
	}
	...
	public static boolean checkIsEnumValue(String val){
		for(SubtopicsStatus subtopicsStatus:SubtopicsStatus.values()){
			if(StringUtils.equals(subtopicsStatus.getEnumVal(), val)){
				return true;
			}
		}
		return false;
	...
	public static String getEnumShowByEnumVal(String val){
		for(SubtopicsStatus subtopicsStatus:SubtopicsStatus.values()){
			if(StringUtils.equals(subtopicsStatus.getEnumVal(), val)){
				return subtopicsStatus.getEnumShow();
			}
		}
		return null;
	}
